...
Status: **proposed, 2026-09-16, agent.** The owner approved the approach
("let's do the conviction scaling this way then, i prefer that then having to
set high and low lambdas arbitrarily"), but it changes `SUBSTRATE` section 4 and
`params/constraints.yaml`, which an agent may not originate (section 2). Three
questions at the end need answering before anything is transcribed.
...
1. **Which statistic.** Median, mean, or a high quantile of the maximum-drawdown
   distribution. This changes the budget by a factor of nearly two and it is a
   mandate question, not a technical one. Note that 0003's existing `3.0x`
   multiple is the median reading, so keeping the median is the status quo.
2. **Whether the derived budget may exceed the 6% mandate.** Section 4 permits
   "episodic excursions, not rewarded on average", and the `tracking_error` veto
   reads the 95th percentile of trailing-3y readings (`decisions/0003`), so there
   is room for the budget to exceed 6% in high-conviction periods without
   breaching the mandate test. The alternative is to cap the derived budget at
   6%, in which case the mechanism only ever tightens.
3. **Whether `g` is indexed by signal persistence as well as conviction.**
   Recommended, on the evidence above, but it makes the budget depend on a
   measured property of the signal rather than on the optimiser's own output,
   which is a slightly larger change.

Two things follow once those are settled: `decisions/0019`'s two coefficients
are withdrawn rather than filled in, and the ruler audit gains a TE-budget axis
so the mechanism is measured before it governs. The falsification test is the
owner's own: run it, then check realised maximum active drawdown against the
ex-ante tolerance across the ladder. If they diverge, `g` is wrong and the
divergence says how.
